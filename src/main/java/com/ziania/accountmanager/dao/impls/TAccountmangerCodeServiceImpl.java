...
public class TAccountmangerCodeServiceImpl extends BaseServiceImpl implements ITAccountmangerCodeService {
     ...
     */
    @Override
    public Map selectByLike(Map params) throws CommonException {
        Map<String, Object> returnMap = new HashMap<>();
        params.put(TAccountmangerCode.TABLE_NAME, TAccountmangerCode.TABLE_NAME);
        Map<String, Object> countMap = this.countByLike(params);
        if (MapUtils.getInteger(countMap, "total") > 0) {
            List<Map<String, Object>> queryList = getBaseDao().selectByCond("com.ziania.accountmanager.dao.TAccountmangerCodeMapper.selectByLikeCond", params);
            returnMap.put("total", MapUtils.getInteger(countMap, "total"));
            returnMap.put("beans", queryList);
        }
        return returnMap;
     ...
     * @throws CommonException
     */
    public Map countByLike(Map params) throws CommonException {
        Map<String, Object> returnMap = new HashMap<>();
        if (MapUtils.isNotEmpty(params)) {
            params.put(TAccountmangerCode.TABLE_NAME, TAccountmangerCode.TABLE_NAME);
            int total = getBaseDao().queryCount("com.ziania.accountmanager.dao.TAccountmangerCodeMapper.countByLikeCond", params);
            returnMap.put("total", total);
        } else {
            returnMap.put("total", 0);
        }
